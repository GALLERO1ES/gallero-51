      ...
      <div className="grid grid-cols-1 md:grid-cols-2 items-center gap-0 md:gap-16">
        <div className="text-center md:text-left">
          <h1 className={`text-6xl md:text-7xl lg:text-8xl font-bold mb-8 transition-all duration-1000 ${isLoaded ? "opacity-100" : "opacity-0 translate-y-10"}`}>
            <span className="text-white font-kritik relative inline-block text-center">
              GALLERO
            </span>
          </h1>
          ...
          <div className={`flex flex-col sm:flex-row gap-6 justify-center md:justify-start transition-all duration-1000 delay-500 ${isLoaded ? "opacity-100" : "opacity-0 translate-y-10"}`}>
            <Button className="btn-primary text-lg hover:scale-105 transition-transform" size="lg" onClick={() => document.getElementById("products")?.scrollIntoView({
            behavior: "smooth"
          })}>
              Explorar Productos
            </Button>
            <Button variant="outline" className="btn-outline text-lg hover:scale-105 transition-transform" size="lg" onClick={() => document.getElementById("cocktails")?.scrollIntoView({
            behavior: "smooth"
          })}>
              Cócteles
            </Button>
